...
class Logreader:
    ...
    def country(self, callsign):
        ...
        for char in callsign:
            if char.isdigit():
                digits += 1
        if digits == 0 or callsign.isdigit():
            callsign = ""
...
class Converters(Logreader):
    ...
    def repair_config(self):
        new_config = [f"softwareversion={self.softwareversion}\n"]
        config = open(
    ...
    def export_last(self, log):
        ...
        string_adi = ""
        lines_txt = [
            f"\t\t* Generated by Ham-starLog version {self.softwareversion}.\n\t\t* Creation date: {Logreader.repr_date(datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S'))} UTC.\n\t\t* {operator_txt} log.\n\n",
        ]
        lines_adi = [
            f"Generated by Ham-starLog V{self.softwareversion}\n<ADIF_VER:5>3.1.0\n<PROGRAMID:11>Ham-starLog\n<PROGRAMVERSION:{len(self.softwareversion)}>{self.softwareversion}\n<EOH>\n\n",
        ]
        while index < len(log):
            date_adi = ""
            report_tx_txt = ""
...
if params.get("softwareversion") == None:
    converter.old_log()
    converter.repair_config()
if params.get("softwareversion")!=converter.softwareversion:
    converter.repair_config()
...
while True:
    ...
    if command[0] == commands_list[0] or command[0] == commands_list[7]:
        if command[1] == "":
            print('Parameter "band" or "frequency" required.\n')
            set_band = ""
        else:
